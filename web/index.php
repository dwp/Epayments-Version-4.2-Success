			...
			<form method="post" action="'.($_SERVER['PHP_SELF']).'">
				<div class="form-group'.((isset($errorvalue))?' error':'').'">
					<label class="form-label" for="psw">
						'.((isset($errorvalue))?'<span class="error-message" aria-hidden="true">'.$errorvalue.'</span>':'').'
						Password
					</label>
...
if(isset($_COOKIE['accessCookie'])) {
  accessOutput();
	...
	if (!empty($_POST)) {
		...
		if ($psw != '') {
			if ($psw != $password) {
				$errorvalue = 'The password you entered is incorrect';
				output($errorvalue);
			}
			else {
			...
			$errorvalue = 'The password field can not be empty';
			output($errorvalue);
		}
	}
